...
def atualizaReg(conn, nu_id, campo, valor):
...
if cursor.rowcount == 1:
    ...
    if row['IC_EXECUCAO'] == 'P':
        ...
        atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Conectando ao Rede Caixa'.format(datetime.datetime.now()))
        em.connect('L:ibmbr10.coredf.caixa:2301')
        em.wait_for_field()

        if not check(em,17,16,'SELECIONE A OP'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Não conectei no Rede Caixa'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Nao conectou no Rede Caixa")
        else:
            log("Conectei no Rede Caixa")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Conectei no Rede Caixa'.format(datetime.datetime.now()))

        atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Acessando o SIGDU'.format(datetime.datetime.now()))
        em.send_string("448")
        em.send_enter()
        em.wait_for_field()

        if not check(em,14,3,'CICS') or not check(em,24,13,'Informe Usuario') or not check(em,3,70,'RCICS'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Não acessei o SIGDU - 4.48'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Nao acessou o 4.48")
        else:
            log("Aguardando usuario e senha do SIGDU")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Autenticando no SIGDU'.format(datetime.datetime.now()))
        ...
        if not check(em,1,17,'SIGDU  GESTAO DESENVOLVIMENTO URBANO') or not check(em,2,32,'MENU PRINCIPAL') or not check(em,2,3,usuario[1:6]):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Nao autenticou no 4.48, provavelmente erro de senha'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Nao autenticou no 4.48, provavelmente erro de senha")
        else:
            log("Conectei no SIGDU")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Acessei o SIGDU'.format(datetime.datetime.now()))

        atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Acessando o DEMA,M'.format(datetime.datetime.now()))
        em.move_to(22,73)
        ...
        if not check(em,2,71,'EGHPO571') or not check(em,2,23,'DEMA,M') or not check(em,23,1,'INFORME A OPCAO E TECLE <ENTER>'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Nao acessei a tela DEMA,M, verifique se tem acesso'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Nao acessei a tela DEMA,M, verifique se tem acesso")
        else:
            log("Acessei a DEMA,M")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Acessei a DEMA,M'.format(datetime.datetime.now()))

        em.send_pf(10)
        em.wait_for_field()

        if not check(em,16,6,'Codigo Unidade CEF') or not check(em,2,71,'EGHPO571') or not check(em,2,23,'DEMA,M'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Nao acessei o F10 da DEMA,M, verifique se tem acesso'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Nao acessei o F10 da DEMA,M, verifique se tem acesso")
        else:
            log("Acessei ao F10 da DEMA,M")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Acessei o F10 da DEMA,M'.format(datetime.datetime.now()))
        ...
        if not check(em,16,26,row['CP_UNIDADE']) or not check(em,2,71,'EGHPO571') or not check(em,23,1,'TECLE <ENTER> PARA PROSSEGUIR COM A DEMANDA') or estaVazio(em,16,33,45):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar unidade no F10 da DEMA,M'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao informar unidade no F10 da DEMA,M")
        else:
            log("Unidade informada no F10 da DEMA,M")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Unidade informada no F10 da DEMA,M'.format(datetime.datetime.now()))
        ...
        if not check(em,4,2,'Deseja fazer demanda para atividade do grupo G') or not check(em,2,71,'EGHPO571') or not check(em,23,1,'INFORME OS DADOS NECESSARIOS E PRESSIONE <ENTER>'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar unidade no F10 da DEMA,M (segundo <Enter>)'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao informar unidade no F10 da DEMA,M (segundo <Enter>)")
        else:
            log("Unidade informada no F10 da DEMA,M (segundo <Enter>)")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Unidade informada no F10 da DEMA,M (segundo <Enter>)'.format(datetime.datetime.now()))
        ...
        if not check(em,4,51,'S') or not check(em,4,2,'Deseja fazer demanda para atividade do grupo G') or not check(em,2,71,'EGHPO571') or not check(em,5,2,'Atividade para o grupo G'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao selecionar demanda Tipo G'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao selecionar demanda Tipo G")
        else:
            log("Selecionou demanda Tipo G")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Selecionou demanda Tipo G'.format(datetime.datetime.now()))
        ...
        if not estaVazio(em,23,1,45) or not check(em,4,2,'Deseja fazer demanda para atividade do grupo G') or not check(em,2,71,'EGHPO571') or not check(em,5,2,'Atividade para o grupo G'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar a nova demanda Tipo G'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao informar a nova demanda Tipo G")
        else:
            log("Informou nova demanda Tipo G")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Informou nova demanda Tipo G'.format(datetime.datetime.now()))
        ...
        if not estaVazio(em,23,1,45) or estaVazio(em,8,23,45) or estaVazio(em,9,23,45) or estaVazio(em,10,23,45) or estaVazio(em,11,23,45) or not check(em,4,2,'Deseja fazer demanda para atividade do grupo G') or not check(em,2,71,'EGHPO571') or not check(em,13,2,'O servico sera executado por terceiro ou empregado Caixa'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar demanda antiga, produto, linha ou fonte'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao informar demanda antiga, produto, linha ou fonte")
        else:
            log("Informou demanda antiga, produto, linha e fonte")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Informou demanda antiga, produto, linha e fonte'.format(datetime.datetime.now()))
        ...
        if not estaVazio(em,23,1,45) or not check(em,13,61,'T') or not check(em,4,2,'Deseja fazer demanda para atividade do grupo G') or not check(em,2,71,'EGHPO571') or not check(em,5,2,'Atividade para o grupo G'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao Informou demanda sera terceirizada'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao informar Informou demanda sera terceirizada")
        else:
            log("Informou demanda sera terceirizada")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Informou demanda sera terceirizada'.format(datetime.datetime.now()))
        ...
        if row['CP_OBJETO'] == 1:
            ...
            if not check(em,2,72,'EGHPO573') or not check(em,2,30,'IMOVEL') or not check(em,23,1,'INFORME O SISTEMA E TECLE <ENTER>') or not check(em,4,17,'SIAPF') or not check(em,7,3,'Comarca'):
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar objeto'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao informar objeto")
            else:
                log("Informou objeto do Tipo 1")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Informou objeto do Tipo 1'.format(datetime.datetime.now()))
            ...
            if not check(em,2,72,'EGHPO573') or not check(em,2,30,'IMOVEL') or not check(em,4,63,'S/N______________') or not check(em,23,1,'INFORME A IDENTIFICACAO NO CARTORIO DO IMOVEL E TECLE <ENTER>'):
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar o sistema Outros'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao informar o sistema Outros")
            else:
                log("Informou sistema Outros")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Informou sistema Outros'.format(datetime.datetime.now()))
            ...
            or estaVazio(em,19,40,40):
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar a matricula'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao informar a matricula")
            else:
                log("Confirmou o Matricula")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Confirmou a Matricula'.format(datetime.datetime.now()))

            em.send_enter()
            em.wait_for_field()

            if not estaVazio(em,20,54,24) or not check(em,23,1,"INFORME O CEP DO ENDERECO E TECLE <ENTER>"):
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao confirmar a Matricula (segundo enter)'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao confirmar a Matricula (segundo enter)")
            else:
                log("Confirmou o Matricula (segundo enter)")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Confirmou o Matricula (segundo enter)'.format(datetime.datetime.now()))
            ...
            or estaVazio(em,12,60,18):                
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar o CEP'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao informar o CEP")
            else:
                log("CEP informado")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - CEP informado'.format(datetime.datetime.now()))
            ...
            or estaVazio(em,12,33,20) \
            or estaVazio(em,12,60,18):
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar o CEP (segundo <enter>)'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao informar o CEP (segundo <enter>)")
            else:
                log("CEP informado (segundo <enter>)")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - CEP informado (segundo <enter>)'.format(datetime.datetime.now()))

            em.move_to(14,17)
            em.send_string(row['CP_LOG_TIPO1'])

            em.move_to(14,40)
            em.send_string(row['CP_LOG_NOME'])
            ...
            em.move_to(19,40)
            em.send_string(row['CP_LOG_BAIRRO'])
...
########################################################################################################################
            ...
            if not estaVazio(em,23,1,45) or not check(em,2,71,'EGHPO571') or not check(em,15,12,2):
                ...
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Confirmou o Objeto'.format(datetime.datetime.now()))
            ...
            if not check(em,2,72,'EGHPO574') or not check(em,2,30,'EMPREENDIMENTO') or not check(em,23,1,'INFORME OS DADOS NECESSARIOS E PRESSIONE <ENTER>') or not check(em,6,25,'SIAPF'):
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar objeto (segundo <enter>)'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao informar objeto (segundo <enter>)")
            else:
                log("Informou objeto do Tipo 2 (segundo <enter>)")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Informou objeto do Tipo 2 (segundo <enter>)'.format(datetime.datetime.now()))
            ...
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar objeto. Valor não previsto (1 ou 2)'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
        ...
        or not check(em,7,20,str(row['CP_NUM_TIPO_G']).strip().zfill(3)):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao informar dados o Imóvel'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao informar dados o Imóvel")
        else:
            log("Dados do Imóvel informado")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Dados do Imóvel informado'.format(datetime.datetime.now()))
        ...
        or not check(em,6,5,'Empresa'):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao Acessar a Escolha Dirigida'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao Acessar a Escolha Dirigida")
        else:
            log("Acessou Escolha Dirigida")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Acessou Escolha Dirigida'.format(datetime.datetime.now()))
        ...
        em.move_to(18,19)
        em.send_string(row['CP_JUSTIFICATIVA'])
        ...
        or not check(em,5,23,row['CP_CNPJ'][12:14]):
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao fazer Escolha Dirigida da Empresa'.format(datetime.datetime.now()))
            atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
            abort(em, "Erro ao fazer Escolha Dirigida da Empresa")
        else:
            log("Escolha Dirigida da Empresa com sucesso")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Escolha Dirigida da Empresa com sucesso'.format(datetime.datetime.now()))
        ...
        em.move_to(11,16)
        em.send_string(row['CP_CONTATO'])

        em.move_to(12,20)
        em.send_string(row['CP_TELEFONE'])

        em.move_to(13,31)
        em.send_string(row['CP_LOCAL_RETIRADA'])

        em.move_to(16,14)
        em.send_string(row['CP_OBSERVACOES'])

        if simulacao == 1:
            log("Simulacao concluida - OS nao foi aberta")
            atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Simulacao concluida - OS nao foi aberta'.format(datetime.datetime.now()))
        else:
            em.send_pf(2)
            em.wait_for_field()

            print(emulador.string_get(23, 1, 35).strip())
            print(emulador.string_get(4, 21, 19).strip())
            print(emulador.string_get(4, 43, 11).strip())
            ...
            or not check(em,5,18,row['CP_CNPJ'][8:12]) \
            or not check(em,5,23,row['CP_CNPJ'][12:14]):
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - Erro ao Abrir OS'.format(datetime.datetime.now()))
                atualizaReg(conn,nu_id, 'IC_EXECUCAO', 'F')
                abort(em, "Erro ao Abrir OS")
            else:
                log("OS Aberta com sucesso")
                atualizaReg(conn,nu_id, 'DE_PROCESSAMENTO', '{} - OS Aberta com sucesso'.format(datetime.datetime.now()))

            

            atualizaReg(conn,nu_id, 'NU_NOVA_OS', emulador.string_get(4, 21, 35).strip())
